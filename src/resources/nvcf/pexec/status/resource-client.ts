...
import * as requests from "nvidia_cloud_functions_ts/resources/nvcf/pexec/status/request-types";
import { Schemas$NvcfPexecStatusGetResponse } from "nvidia_cloud_functions_ts/types/nvcf-pexec-status-get-response";
...
export class StatusClient extends CoreResourceClient {
  ...
  get(
    ...
    opts?: RequestOptions,
  ): ApiPromise<types.NvcfPexecStatusGetResponse> {
    return this._client.makeRequest({
      method: "get",
      path: `/v2/nvcf/pexec/status/${request.requestId}`,
      responseSchema: Schemas$NvcfPexecStatusGetResponse.in,
      opts,
    });
  }
}
